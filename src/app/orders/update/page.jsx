    ...
    useEffect(() => {
        const fetchOrders = async () => {
            try {
                ...
                const uniqueOrders = Array.from(new Map(res.data.data.map(order => [order._id, order])).values());
                setOrders(uniqueOrders);

                if (uniqueOrders.length > 0) {
                    setSelectedOrderId(uniqueOrders[0]._id);
                    setStatus(uniqueOrders[0].status);
                }
            } catch (error) {
                console.error("Error fetching orders:", error);
            }

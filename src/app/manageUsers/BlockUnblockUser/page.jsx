...
//       const apiUrl = `http://localhost:5000/api/orders/update-item-status`;

//       const productId = itemId.productId
//       console.log("Action:", action); // Check the action that will be used
//       console.log("ID", productId); // Check the action that will be used
//       console.log("API URL:", apiUrl); // Check the API URL being used

//       // Send the PUT request to update the status
//       const response = await axios.put(apiUrl, { productId, action });
...
//               <td
//                 className={`border p-2 font-semibold ${user?.items[0].status === "Active"
//                   ? "text-green-600"
//                   : "text-red-600"
//                   }`}
//               >
...
//                     )
//                   }
//                   className={`px-4 py-2 rounded text-white ${user.userId.
//                     status
...
export default function Page() {
  ...
  const toggleBlockStatus = async (itemId, currentStatus) => {
    ...
    try {
      ...
      const action =
        currentStatus.toLowerCase() === "active" ? "block" : "active";
      const apiUrl = `http://localhost:5000/api/orders/update-item-status/${itemId}`;
            ...
            >
              ...
              <td className="border p-2">
                <button
                  ...
                  className={`px-4 py-2 rounded text-white ${user.status === "Active" ? "bg-red-500" : "bg-green-500"
                    }`}
                >
